 ...
 * VSOP87Calculator
 * 
 * VSOP87Calculator 是以 VSOP87 为基础的行星日心坐标数据计算器
 *
...
class VSOP87Calculator {
  ...
  calc(dataArray, tNumsArray) {
    ...
    for (var i = 0; i < dataArray.length; i++) {
      ...
      for (var j = 0; j < tNum; j++) {
        sum += dataArray[i][j][0] * Math.cos(dataArray[i][j][1] + dataArray[i][j][2] * jdr.JDET);
      }
  ...
  estimateMaxError(dataArray, tNumsArray) {
    ...
    for (var i = 0; i < dataArray.length; i++) {
      ...
      accuracy += Math.sqrt(n) * A * jdr.JDETP(i);
    }
  ...
  makeTruncationNums(dataArray, maximumError) {
    ...
    while(1) {
      ...
      let accuracy = Math.sqrt(tNum) * A * jdr.JDETP(item);
